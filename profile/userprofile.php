				    ...
				    <tr>
						<td colspan="2">
							<div style="margin:10px;">
								...
								<input type="submit" name="action" value="<?php echo (isset($LANG['SUBMIT_EDITS'])?$LANG['SUBMIT_EDITS']:'Submit Edits'); ?>" />
							</div>
						</td>
					</tr>
				</table>
			...
			<fieldset style='padding:15px;width:500px;'>
		    	...
		    	<table>
		    		...
		    		<tr>
		    			<td colspan="2">
							<input type="hidden" name="userid" value="<?php echo $userId;?>" />
							<input type="submit" name="action" value="<?php echo (isset($LANG['CHANGE_PASSWORD'])?$LANG['CHANGE_PASSWORD']:'Change Password'); ?>" />
		    			</td>
		    		</tr>
				</table>
			</fieldset>
	...
	<div id="logineditdiv" style="display:none;margin:15px;">
		<fieldset style='padding:15px;width:500px;'>
			...
			<form name="modifyloginform" action="viewprofile.php" method="post" onsubmit="return verifyModifyLoginForm(this);">
				...
				<div style="margin:10px">
					<input type="hidden" name="userid" value="<?php echo $userId;?>" />
					<input type="submit" name="action" value="<?php echo (isset($LANG['CHANGE_USERNAME'])?$LANG['CHANGE_USERNAME']:'Change Username'); ?>" />
				</div>
			</form>
		</fieldset>
	</div>
    <div id="managetokensdiv" style="display:none;margin:15px;">
        <fieldset style='padding:15px;width:500px;'>
            ...
            <form name="cleartokenform" action="viewprofile.php" method="post" onsubmit="">
                <div><?php echo (isset($LANG['YOU_HAVE'])?$LANG['YOU_HAVE']:'You currently have').' <b>'.($tokenCount?$tokenCount:0).' </b>'.
				(isset($LANG['EXPLAIN_TOKENS'])?$LANG['EXPLAIN_TOKENS']:'access tokens linked to your account. Tokens are created when you select "Remember Me" 
				when logging in, or access the portal from an external app. If the number of access tokens you have seems high, please click on the button below 
				to clear the tokens linked to your account.'); ?>
                </div>
                <div style="margin:10px">
                    <input type="hidden" name="userid" value="<?php echo $userId;?>" />
                    <input type="submit" name="action" value="<?php echo (isset($LANG['CLEAR_TOKENS'])?$LANG['CLEAR_TOKENS']:'Clear Tokens'); ?>" />
                </div>
            </form>
        </fieldset>
    </div>
		...
		<div id="addtaxonrelationdiv" style="display:none;">
			<fieldset style="padding:20px;margin:15px;">
				...
				<div style="margin-bottom:10px;">
					<?php echo (isset($LANG['TAX_FORM'])?$LANG['TAX_FORM']:'Use this form to define a new taxon-based region of interest. Contact portal 
					administrators for assignment of new taxon specific Occurrence Identification and Taxonomic Thesaurus editing status.'); ?>
				</div>
				<form name="addtaxonomyform" action="viewprofile.php" method="post" onsubmit="return verifyAddTaxonomyForm(this)">
					...
					<div style="margin:20px 10px;">
						<input name="action" type="submit" value="<?php echo (isset($LANG['ADD_TAX'])?$LANG['ADD_TAX']:'Add Taxonomic Relationship'); ?>" />
					</div>
				</form>
			</fieldset>
		</div>
