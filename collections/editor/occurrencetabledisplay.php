...
if($SYMB_UID){
	...
	if($recStart >= $recLimit){
		$navStr .= '<a href="#" onclick="return submitQueryForm('.($recStart-$recLimit).');" title="'<?php echo (isset($LANG['PREVIOUS'])?$LANG['PREVIOUS']:'Previous 1000 records'); ?>'">&lt;&lt;</a>';
	}
	$navStr .= ' | ';
	...
	<div id="innertext">
		...
		if(($isEditor || $crowdSourceMode)){
			...
			<div style="text-align:right;width:790px;margin:-30px 15px 5px 0px;">
				<button title="Search / Filter" onclick="toggleSearch();return false;">Search Form</button>
				<?php
