...
if($links){
	foreach($links as $linkKey => $linkUrl){
		if($linkUrl['title'] == 'REDIRECT'){
			$locUrl = str_replace('--SCINAME--',rawurlencode($taxonManager->getDisplayName()),$linkUrl['url']);
...
<html>
<head>
	<title><?php echo $DEFAULT_TITLE." - ".$taxonManager->getDisplayName(); ?></title>
	<meta http-equiv="Content-Type" content="text/html; charset=<?php echo $CHARSET; ?>"/>
	<?php
	if(file_exists($SERVER_ROOT.'/includes/spprofilehead.php')){
		include_once($SERVER_ROOT.'/includes/spprofilehead.php');
	}
	else{
		?>
    <?php
    ...
    ?>
		<link href="<?php echo $CLIENT_ROOT; ?>/css/speciesprofilebase.css?ver=1" type="text/css" rel="stylesheet" />
		<link href="<?php echo $CLIENT_ROOT; ?>/css/speciesprofile.css?ver=1" type="text/css" rel="stylesheet" />
		<?php
	}
	?>
	...
	<style type="text/css">
		.displayName{ font-weight:bold; color:#990000; font-size:16px; <?php if($taxonManager->getRankId() > 179) echo 'font-style: italic;'; ?> }
		.displayAuthor{ color:#990000; font-size:16px; }
	</style>
</head>
<body>
<?php
...
<div id="popup-innertext">
	<?php
	if($taxonManager->getDisplayName()){
		if(count($taxonManager->getAcceptedArr()) == 1){
			...
			if($taxonRank > 180){
				...
				<tr>
					...
					<div style='float:left;margin-left:10px;'>
						<span class="displayName"><?php echo $taxonManager->getDisplayName(); ?></span>
						<span class="displayAuthor"><?php echo $taxonManager->getDisplayAuthor(); ?></span>
						<?php
					...
					*/
					...
					if($links && $links[0]['sortseq'] == 1){
						$uStr = str_replace('--SCINAME--',rawurlencode($taxonManager->getDisplayName()),$links[0]['url']);
						?>
				...
				<tr>
					<td width="300" valign="top">
						<div id="family" style="margin-left:25px;">
							<?php echo '<b>'.(isset($LANG['FAMILY'])?$LANG['FAMILY']:'Family').':</b> '.$taxonManager->getDisplayFamily(); ?>
						</div>
						<?php
					...
					<td colspan="2">
						<div id="img-div" style="height:300px;overflow:hidden;">
							...
							if($url){
								...
								echo '<img src="'.$url.'" title="'.$taxonManager->getDisplayName().'" alt="'.$taxonManager->getDisplayName().'" />';
				...
				<tr>
					<td style="width:250px;vertical-align:top;">
						<?php
						$displayName = $taxonManager->getDisplayName();
						if($taxonRank > 140){
							$parentLink = "index.php?tid=".$taxonManager->getParentTid()."&clid=".$clid."&pid=".$pid."&taxauthid=".$taxAuthId;
						...
						echo '<div class="displayName" style="margin-top:15px;margin-left:10px;">'.$displayName.'</div>';
						if($taxonRank > 140) echo "<div id='family' style='margin-top:3px;margin-left:20px;'><b>Family:</b> ".$taxonManager->getDisplayFamily()."</div>\n";
						if(!$taxonManager->echoImages(0,1,0)){
							echo "<div class='image' style='width:260px;height:260px;border-style:solid;margin-top:5px;margin-left:20px;text-align:center;'>";
							if($isEditor){
								echo '<a href="profile/tpeditor.php?category=imageadd&tid='.$taxonManager->getTid().'"><b>'.(isset($LANG['ADD_IMAGE'])?$LANG['ADD_IMAGE']:'Add an Image').'</b></a>';
										...
										if(isset($GOOGLE_MAP_THUMBNAILS) && $GOOGLE_MAP_THUMBNAILS){
											...
											if($taxonManager->getRankId() > 140){
												...
												if(array_key_exists("map",$subArr) && $subArr["map"]){
													echo '<img src="'.$subArr['map'].'" title="'.$taxonManager->getDisplayName().'" alt="'.$taxonManager->getDisplayName().'" />';
												}
												else{
			...
			?>
			<div class="displayName" style="float:left"><?php echo $taxonManager->getDisplayName(); ?></div>
			<div style="float:left">
				<div>Status: not accepted</div>
				<div>
					<?php
			...
			<div style="float:left"><?php echo $taxonManager->getDescriptionTabs(); ?></div>
			<?php
		}
	}
	else{
