...
class TaxonomyEditorManager extends Manager{
	...
	public function submitTaxonEdits($postArr){
		...
		if(!$this->conn->query($sql)){
			$statusStr = (isset($LANG['ERROR_EDITING_TAXON'])?$LANG['ERROR_EDITING_TAXON']:'ERROR editing taxon').': '.$this->conn->error;
		}
	...
	public function submitTaxStatusEdits($parentTid,$tidAccepted){
		...
		if(is_numeric($parentTid) && is_numeric($tidAccepted)){
			...
			if($this->conn->query($sql)){
				...
				$status = (isset($LANG['UNABLE_EDIT_TAX'])?$LANG['UNABLE_EDIT_TAX']:'Unable to edit taxonomic placement. SQL').': '.$sql;
			}
		}
		return $status;
	}
	...
	public function submitSynonymEdits($targetTid, $tidAccepted, $unacceptabilityReason, $notes, $sortSeq){
		...
		if(is_numeric($tidAccepted)){
			...
			//echo $sql; exit();
			if(!$this->conn->query($sql)){
				$statusStr = (isset($LANG['ERROR_SYN_EDITS'])?$LANG['ERROR_SYN_EDITS']:'ERROR submitting synonym edits').': '.$this->conn->error;
			}
		}
		return $statusStr;
	}
	...
	public function submitAddAcceptedLink($tidAcc, $deleteOther = true){
		...
		if(is_numeric($tidAcc)){
			...
			//echo $sql;
			if(!$this->conn->query($sql)){
				$statusStr = (isset($LANG['ERROR_ADD_LINK'])?$LANG['ERROR_ADD_LINK']:'ERROR adding accepted link').': '.$this->conn->error;
			}
		}
		return $statusStr;
	}
	...
	public function removeAcceptedLink($tidAccepted){
		...
		if(is_numeric($tidAccepted)){
			...
			if(!$this->conn->query($sql)){
				$statusStr = (isset($LANG['ERROR_REMOVING_LINK'])?$LANG['ERROR_REMOVING_LINK']:'ERROR removing tidAccepted link').': '.$this->conn->error;
			}
		}
		return $statusStr;
	}
		...
		if(is_numeric($tid)){
			...
			if($switchAcceptance && is_numeric($tidAccepted)){
				...
				if(!$this->conn->query($sqlSwitch)){
					$statusStr = (isset($LANG['ERROR_CHANGING_ACCEPTED'])?$LANG['ERROR_CHANGING_ACCEPTED']:'ERROR changing to accepted').': '.$this->conn->error;
				}
	...
	public function submitChangeToNotAccepted($tid,$tidAccepted,$reason,$notes){
		...
		if(is_numeric($tid) && is_numeric($tidAccepted)){
			...
			//echo $sql;
			if(!$this->conn->query($sql)){
				$status = (isset($LANG['ERROR_SWITCH_ACCEPT'])?$LANG['ERROR_SWITCH_ACCEPT']:'ERROR: unable to switch acceptance').'; '.$this->conn->error;
				$status .= '<br/>SQL: '.$sql;
			}
			else{
				...
				if(!$this->conn->query($sqlSyns)){
					$status = (isset($LANG['ERROR_LINK_SYNONYMS'])?$LANG['ERROR_LINK_SYNONYMS']:'ERROR: unable to transfer linked synonyms to accepted taxon').'; '.$this->conn->error;
				}
	...
	public function rebuildHierarchy($tid = 0){
		...
		if($this->rankid > 140){
			...
			if(!$this->conn->query($sql)){
				$this->errorMessage = (isset($LANG['ERROR_RESET_FAMILY'])?$LANG['ERROR_RESET_FAMILY']:'ERROR attempting to reset family string').': '.$this->conn->error;
		...
		if($this->conn->query($sqlTaxa)){
			...
			if($parTid){
				...
				if(!$this->conn->query($sqlTaxStatus)){
					return (isset($LANG['ERROR_LOAD_TAXSTATUS'])?$LANG['ERROR_LOAD_TAXSTATUS']:'ERROR: Taxon loaded into taxa, but failed to load taxstatus').': '.$this->conn->error.'; '.$sqlTaxStatus;
				}
				...
				if($this->conn->query($sqlEnumTree)){
					...
					if(!$this->conn->query($sqlEnumTree2)){
						echo (isset($LANG['WARNING_TAXAENUMTREE2'])?$LANG['WARNING_TAXAENUMTREE2']:'WARNING: Taxon loaded into taxa, but failed to populate taxaenumtree(2)').': '.$this->conn->error;
					}
				}
				else{
					echo (isset($LANG['WARNING_TAXAENUMTREE'])?$LANG['WARNING_TAXAENUMTREE']:'WARNING: Taxon loaded into taxa, but failed to populate taxaenumtree').': '.$this->conn->error;
				}
			}
			else{
				return (isset($LANG['ERROR_MISSING_PARENTID'])?$LANG['ERROR_MISSING_PARENTID']:'ERROR loading taxon due to missing parentTid');
			}
			...
			if(!$this->conn->query($sql1)){
				echo (isset($LANG['WARNING_OCCURRENCES_NOT UPDATED'])?$LANG['WARNING_OCCURRENCES_NOT']:'WARNING: Taxon loaded into taxa, but  occurrences must be updated with matching name').': '.$this->conn->error;
			}
			...
			if($occidArr){
				...
				if(!$this->conn->query($sql2)){
					echo (isset($LANG['WARNING_UPDATE_IMAGES'])?$LANG['WARNING_UPDATE_IMAGES']:'WARNING: Taxon loaded into taxa, but occurrence images must be updated with matching name').': '.$this->conn->error;
				}
			}
			...
			$this->errorMessage = (isset($LANG['ERROR_INSERT'])?$LANG['ERROR_INSERT']:'ERROR inserting new taxon').': '.$this->conn->error;
	...
	public function transferResources($targetTid){
		...
		if(is_numeric($targetTid)){
			...
			if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_IMGS'])?$LANG['ERROR_TRANSFER_IMGS']:'ERROR transferring image links').' ('.$this->conn->error.')';

			//Vernaculars
			$sql ='UPDATE IGNORE taxavernaculars SET tid = '.$targetTid.' WHERE tid = '.$this->tid;
			if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_VERNACULARS'])?$LANG['ERROR_TRANSFER_VERNACULARS']:'ERROR transferring vernaculars').' ('.$this->conn->error.')';

			//Text Descriptions
			$sql ='UPDATE IGNORE taxadescrblock SET tid = '.$targetTid.' WHERE tid = '.$this->tid;
			if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_TAXADESC'])?$LANG['ERROR_TRANSFER_TAXADESC']:'ERROR transferring taxadescblocks').' ('.$this->conn->error.')';

			//Vouchers and checklists
			$sql ='UPDATE IGNORE fmchklsttaxalink SET tid = '.$targetTid.' WHERE tid = '.$this->tid;
			if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_CHECKLIST'])?$LANG['ERROR_TRANSFER_CHECKLIST']:'ERROR transferring checklist links').' ('.$this->conn->error.')';

			$sql ='UPDATE IGNORE fmvouchers SET tid = '.$targetTid.' WHERE tid = '.$this->tid;
			if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_VOUCHERS'])?$LANG['ERROR_TRANSFER_VOUCHERS']:'ERROR transferring vouchers').' ('.$this->conn->error.')';

			$sql ='DELETE FROM fmvouchers WHERE tid = '.$this->tid;
			if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_LVOUCHERS'])?$LANG['ERROR_TRANSFER_LVOUCHERS']:'ERROR deleting leftover vouchers').' ('.$this->conn->error.')';

			$sql ='DELETE FROM fmchklsttaxalink WHERE tid = '.$this->tid;
			if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_LCHECKLISTS'])?$LANG['ERROR_TRANSFER_LCHECKLISTS']:'ERROR deleting leftover checklist links').' ('.$this->conn->error.')';

			//Key descriptions
			$sql ='UPDATE IGNORE kmdescr SET tid = '.$targetTid.' WHERE inherited IS NULL AND tid = '.$this->tid;
			if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_MORPHOLOGY'])?$LANG['ERROR_TRANSFER_MORPHOLOGY']:'ERROR transferring morphology for ID key').' ('.$this->conn->error.')';

			//Taxon links
			$sql ='UPDATE IGNORE taxalinks SET tid = '.$targetTid.' WHERE tid = '.$this->tid;
			if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_TAXLINKS'])?$LANG['ERROR_TRANSFER_TAXLINKS']:'ERROR transferring taxa links').' ('.$this->conn->error.')';

			//Transfer children taxa
			$sql ='UPDATE taxstatus SET parenttid = '.$targetTid.' WHERE parenttid = '.$this->tid;
			if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_CHILD'])?$LANG['ERROR_TRANSFER_CHILD']:'ERROR transferring child taxa').' ('.$this->conn->error.')';

			//Transfer Synonyms
			$sql ='UPDATE taxstatus SET tidaccepted = '.$targetTid.' WHERE tidaccepted = '.$this->tid;
			if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_SYN'])?$LANG['ERROR_TRANSFER_SYN']:'ERROR transferring synonyms taxa').' ('.$this->conn->error.')';

			//Adjust taxaEnumTree index table
			$sql ='UPDATE taxaenumtree SET parenttid = '.$targetTid.' WHERE parenttid = '.$this->tid;
			if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_TAXENUMTREE'])?$LANG['ERROR_TRANSFER_TAXENUMTREE']:'ERROR resetting taxaEnumTree index').' ('.$this->conn->error.')';
	...
	public function deleteTaxon(){
		...
		if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_SETTING_NULL'])?$LANG['ERROR_SETTING_NULL']:'ERROR setting tid to NULL for occurrence images in deleteTaxon method').' ('.$this->conn->error.')';
		...
		*/
		...
		if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_DEL_VERNACULARS'])?$LANG['ERROR_DEL_VERNACULARS']:'ERROR deleting vernaculars in deleteTaxon method').' ('.$this->conn->error.')';

		//Text Descriptions
		$sql ='DELETE FROM taxadescrblock WHERE tid = '.$this->tid;
		if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_DEL_TAXDESC'])?$LANG['ERROR_DEL_TAXDESC']:'ERROR deleting taxa description blocks in deleteTaxon method').' ('.$this->conn->error.')';
		...
		if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TIDINT_NULL'])?$LANG['ERROR_TIDINT_NULL']:'ERROR setting tidinterpreted to NULL in deleteTaxon method').' ('.$this->conn->error.')';

		$sql ='UPDATE omoccurdeterminations SET tidinterpreted = NULL WHERE tidinterpreted = '.$this->tid;
		if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_TRANSFER_DETS'])?$LANG['ERROR_TRANSFER_DETS']:'ERROR transferring occurrence determination records').' ('.$this->conn->error.')';

		//Vouchers
		$sql ='DELETE FROM fmvouchers WHERE tid = '.$this->tid;
		if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_DEL_VOUCHER'])?$LANG['ERROR_DEL_VOUCHER']:'ERROR deleting voucher links in deleteTaxon method').' ('.$this->conn->error.')';

		//Links to checklists
		$sql ='DELETE FROM fmchklsttaxalink WHERE tid = '.$this->tid;
		if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_DEL_CHECKLIST'])?$LANG['ERROR_DEL_CHECKLIST']:'ERROR deleting checklist links in deleteTaxon method').' ('.$this->conn->error.')';

		//Key descriptions
		$sql ='DELETE FROM kmdescr WHERE tid = '.$this->tid;
		if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_DEL_MORPHOLOGY'])?$LANG['ERROR_DEL_MORPHOLOGY']:'ERROR deleting morphology for ID Key in deleteTaxon method').' ('.$this->conn->error.')';

		//Taxon links
		$sql ='DELETE FROM taxalinks WHERE tid = '.$this->tid;
		if(!$this->conn->query($sql)) $this->warningArr[] = (isset($LANG['ERROR_DEL_TAXLINKS'])?$LANG['ERROR_DEL_TAXLINKS']:'ERROR deleting taxa links in deleteTaxon method').' ('.$this->conn->error.')';
		...
		if($this->conn->query($sql)){
			...
			if(!$this->conn->query($sql)){
				$this->errorMessage = (isset($LANG['ERROR_ATTEMPT_DELETE'])?$LANG['ERROR_ATTEMPT_DELETE']:'ERROR attempting to delete taxon').': '.$this->conn->error;
				$status = false;
				...
				foreach($taxStatusArr as $taxAuthId => $tsArr){
					...
					if(!$this->conn->query($tsNewSql)) $this->warningArr[] = (isset($LANG['ERROR_REESTABLISH'])?$LANG['ERROR_REESTABLISH']:'ERROR attempting to re-establish taxon').' ('.$this->conn->error.')';
				}
			}
		}
		else{
			$this->warningArr[] = (isset($LANG['ERROR_DEL_STATUS'])?$LANG['ERROR_DEL_STATUS']:'ERROR attempting to delete taxon status').': '.$this->conn->error;
		}
